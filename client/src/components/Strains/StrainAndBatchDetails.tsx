...
export const StrainAndBatchDetails: React.FC<Props> = ({strainAndBatch}: Props) => {
    const strainType = useStoreState(state => state.strainTypes?.find(st => st.id === strainAndBatch.strain.strainTypeId));
        ...
        <Card className="mb-4">
            <CardHeader>
                <h5 className="mb-0">
                    ...
                    <StrainTypeBadge typeName={strainType?.name || ""} /></h5>
            </CardHeader>
