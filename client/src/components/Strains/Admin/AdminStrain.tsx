...
export const AdminStrain: React.FC<Props> = ({ strain }: Props) => {
    const strainType = useStoreState(state => state.strainTypes)?.find(
        st => st.id === strain.strainTypeId
    );
    const strainBatches = useStoreState(state => state.batches)?.filter(
        b => b.strainId === strain.id
    );
            ...
            <React.Fragment>
                {strainType ? (
                    <CardBody className="p-0">
                        <KeyValTable
                            className="table-striped card-table same-width mb-0"
                            keyValPairs={[
                                {
                                    key: "Strain Type",
                                    val: (
                                        <StrainTypeBadge
                                            typeName={strainType.name}
                                        />
                                    ),
                                },
